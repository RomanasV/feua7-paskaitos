...
let task371 = arr.filter(item => isNaN(item) && item.endsWith('d'));
console.log(task371);

// 3.72. Gauti tik tekstus (string), kurie baigiasi raide s.
let task372 = arr.filter(item => isNaN(item) && item.endsWith('s'));
console.log(task372);

// 3.73. Gauti tik tekstus (string), kurie turi daugiau nei 4 simbolius ir turi raidę o.
let task373 = arr.filter(item => isNaN(item) && item.length > 4 && item.includes('o'));
console.log(task373);
